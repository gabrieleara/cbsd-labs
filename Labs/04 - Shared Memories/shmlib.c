...
struct shared_memory
{
    ...
    unsigned char data[SHM_BUFFER_SIZE]; // Values

    int rear;  // Consuming pointer
    int front; // Producing pointer

    int how_many_free;
    int how_many_data;

    bool expect_more_data; // It will tell the consumer to expect more data to
                           // come
...
static pthread_condattr_t  cond_attrs;

static pthread_once_t attributes_are_initialized = PTHREAD_ONCE_INIT;
extern int initialize_random();

static void _attr_init()
{
